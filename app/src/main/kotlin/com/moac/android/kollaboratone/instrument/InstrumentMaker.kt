...
class InstrumentMaker(val width: Int, val height: Int) {
    ...
    fun createInstrumentRunner() = InstrumentRunner(createInstrument(), createInstrumentRenderer())

    private fun createInstrumentRenderer() = InstrumentRenderer(createPaint())

    private fun createInstrument() = Instrument(createArena(), createNotePlayer())
    ...
    private fun createInitialBalls(): Collection<Ball> {
        ...
        val b1: Ball = Ball("1", OctaveNote(KEY.get(0), 0), COLORS.get(0), 150F, widthSegment.toFloat(), halfHeight, Velocity(0, 20, Direction.NONE, Direction.UP), false)
        val b2: Ball = Ball("2", OctaveNote(KEY.get(2), 0), COLORS.get(2), 100F, (widthSegment * 2).toFloat(), halfHeight, Velocity(0, 40, Direction.NONE, Direction.DOWN), false)
        val b3: Ball = Ball("3", OctaveNote(KEY.get(4), 0), COLORS.get(4), 100F, (widthSegment * 3).toFloat(), halfHeight, Velocity(0, 40, Direction.NONE, Direction.UP), false)
        val b4: Ball = Ball("4", OctaveNote(KEY.get(6), 0), COLORS.get(6), 100F, (widthSegment * 4).toFloat(), halfHeight, Velocity(0, 40, Direction.NONE, Direction.DOWN), false)
